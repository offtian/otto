...
class TestInMemoryApprovalStore:
    ...
    async def test_resolve_transitions_a_pending_approval(self):
        # Given a stored pending approval
        store = approvals.InMemoryApprovalStore()
        await store.save(_pending(entities.SlackThread(channel_id="C1", thread_ts="1.0")))

        # When it is resolved as approved
        resolved = await store.resolve("ap-1", approvals.ApprovalStatus.APPROVED)

        # Then the returned and stored approval are both terminal
    ...
    async def test_resolve_raises_when_already_resolved(self):
        ...
        store = approvals.InMemoryApprovalStore()
        await store.save(_pending(entities.SlackThread(channel_id="C1", thread_ts="1.0")))
        await store.resolve("ap-1", approvals.ApprovalStatus.DENIED)
        ...
        with pytest.raises(approvals.ApprovalAlreadyResolved):
            await store.resolve("ap-1", approvals.ApprovalStatus.APPROVED)
