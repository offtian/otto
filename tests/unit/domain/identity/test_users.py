...
class TestUserDirectory:
    ...
    def test_same_person_matches_across_channels(self):
        ...
        same = directory.same_person("U_ALICE", "JIRA_ALICE")

        # Then they are recognized as one person (T3 cross-channel)
        assert same

    def test_same_person_rejects_different_people(self):
        # Given a directory with two distinct users
        directory = users.UserDirectory(users=[ALICE, BOB])

        # When their ids are compared
        same = directory.same_person("U_ALICE", "U_BOB")

        # Then they are not the same person
        assert not same

    def test_unmapped_ids_only_match_themselves(self):
        # Given an empty directory
        directory = users.UserDirectory(users=[])

        # When an unmapped id is compared with itself and with another
        identical = directory.same_person("U_X", "U_X")
        different = directory.same_person("U_X", "U_Y")
